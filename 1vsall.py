...
def compare_frames_change_detection(prev_frame, next_frame, min_contour_area):
    ...
    if score == 0:
        #os.remove(images_path+'/'+img_name2)
        duplicates.append(img_name1+' is similar to '+img_name2)
    #cv2.imwrite(threshold_images_path+'/'+img_name1+'_'+img_name2+'_'+str(score)+'.png',thresh)


for img1, img2 in itertools.combinations(contents,2):
    img_name1 = img1
    img_name2 = img2
